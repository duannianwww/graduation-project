...
<body style="position: relative;left:20%;margin-top: 100px;height:180px;margin-left: -130px ">
    <table class="sea">
        ...
        <tr>
            ...
            <td><%=book1.getPress()%></td>
            <td><%=book1.getPulishtime()%></td>
            <td><%=book1.getIntroduct()%></td>
...
<%--            <td><button type="button" onclick="window.open('http://localhost:8080/Login_war_exploded/video/<%=book1.getFilename()%>')">审核视频</button></td>--%>
            <script type="text/javascript">
                $(document).ready(function() {
                    $("#<%=book1.getBookname()%>update").click(function() {
                        $.ajax({
                            url:"updateVServlet",
                            dataType:"json",
                            data:{
                ...
                $(document).ready(function() {
                    $("#<%=book1.getBookname()%>delete").click(function() {
                        $.ajax({
                            type:"post",
                            url:"deleteServlet",
                            data:{
                ...
                $(document).ready(function() {
                    $("#<%=book1.getBookname()%>videoliulan").click(function() {
                        $.ajax({
                            url:"VideoliulanServlet",
                            dataType:"json",
                            data:{
                                bookname:"<%=book1.getBookname()%>",
                            },
                            ...
                            success:function (data) {
                                ...
                                {
                                    alert(data.msg);
                                    window.location.href ="video_search.jsp"
                                }
                                else
                                {
                                    alert(data.msg);
                ...
                $(document).ready(function() {
                    $("#<%=book1.getBookname()%>pinlunliulan").click(function() {
                        $.ajax({
                            url:"VideopinlunServlet",
                            dataType:"json",
                            data:{
                                bookname:"<%=book1.getBookname()%>",
                                pinluntablename:"<%=book1.getFilenamepinlun()%>",
                            },
                            type:"post",
                            success:function (data) {
                                if(data.flag)
                                {
                                    alert(data.msg);
                                    window.location.href ="video_pinlunliulan.jsp"
                                }
                                else
                                {
                                    alert(data.msg);
            ...
            <td><button type="button" class="update" id="<%=book1.getBookname()%>pinlunliulan">浏览评论页</button></td>
            <td><button type="button" class="update" id="<%=book1.getBookname()%>videoliulan">浏览视频页</button></td>
            <td><button type="button" class="update" id="<%=book1.getBookname()%>update">修改</button></td>
            <td><button type="button" class="delete" id="<%=book1.getBookname()%>delete">删除</button></td>
        </tr>
        <%}
        %>
        </tbody>
